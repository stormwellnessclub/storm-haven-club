...
export function NonMemberSmsOptInGate() {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      if (email) {
        const { data: pImp } = await supabase
          ...
          .limit(1)
          .maybeSingle();
        if (pImp?.phone) found = pImp.phone;
        if (!found) {
      ...
      >
        <DialogHeader>
          ...
          <DialogDescription className="text-center pt-2">
            We text class reminders, waitlist alerts, and spa appointment
            confirmations. Confirm your number to continue using the portal.
          </DialogDescription>
        </DialogHeader>

        <div className="space-y-2">
          <Label htmlFor="nm-sms-gate-phone">Mobile number</Label>
          <Input
            id="nm-sms-gate-phone"
            type="tel"
            inputMode="tel"
            autoComplete="tel"
