...
BEGIN
  ...
  FROM (
    SELECT
      ...
      -- prior completed bookings in SAME class type (excluding this booking)
      (
        SELECT COUNT(*)::int
        FROM class_bookings pcb
        JOIN class_sessions pcs ON pcs.id = pcb.session_id
        WHERE pcb.status = 'completed'
          AND pcb.id <> cb.id
      ...
      -- total completed bookings all-time (excluding this booking)
      (
        SELECT COUNT(*)::int
        FROM class_bookings tcb
        WHERE tcb.status = 'completed'
          AND tcb.id <> cb.id
          AND (
  ...
  -- Post-process: add is_first_in_type, total_classes (including current if checked-in), milestone_hit
  SELECT coalesce(jsonb_agg(
    elem
    || jsonb_build_object(
      'is_first_in_type', ((elem->>'prior_in_type')::int = 0),
      'total_classes', (
        (elem->>'prior_total')::int
        + CASE WHEN (elem->>'status') = 'completed' OR (elem->>'checked_in_at') IS NOT NULL THEN 1 ELSE 0 END
      ),
      'milestone_hit', (
        CASE WHEN (elem->>'status') = 'completed' OR (elem->>'checked_in_at') IS NOT NULL
          THEN ((elem->>'prior_total')::int + 1) = ANY(v_milestones)
          ELSE false
        END
      ),
      'next_milestone', (
