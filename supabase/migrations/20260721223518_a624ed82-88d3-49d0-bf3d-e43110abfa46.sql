...
BEGIN
  ...
  LOOP
    ...
    v_results := v_results || jsonb_build_object(
      'id', 'member-' || r.id, 'type', 'member',
      'name', r.first_name || ' ' || r.last_name,
      'time', r.checked_in_at, 'subtitle', r.membership_type,
  ...
  FOR r IN
    SELECT id, guest_name, used_at FROM public.guest_passes
    WHERE status = 'used' AND used_at >= v_today_start AND used_at < v_today_end
  LOOP
    v_guest_count := v_guest_count + 1;
    v_results := v_results || jsonb_build_object(
      'id', 'guest-' || r.id, 'type', 'guest', 'name', r.guest_name,
      'time', r.used_at, 'subtitle', 'Guest Pass', 'sub_type', 'Guest Pass'
    );
  END LOOP;
  ...
  FOR r IN
    ...
    LEFT JOIN public.profiles p ON p.id = cb.user_id AND cb.member_id IS NULL
    WHERE cb.checked_in_at IS NOT NULL
      AND cb.checked_in_at >= v_today_start AND cb.checked_in_at < v_today_end
  LOOP
    IF r.m_first IS NOT NULL THEN
      v_name := r.m_first || ' ' || r.m_last; v_sub_type := 'Member';
    ELSIF r.nmp_first IS NOT NULL THEN
      v_name := r.nmp_first || ' ' || r.nmp_last; v_sub_type := 'Non-Member';
    ELSIF r.p_first IS NOT NULL THEN
      v_name := r.p_first || ' ' || COALESCE(r.p_last,''); v_sub_type := 'Non-Member';
    ELSIF r.walk_in_name IS NOT NULL THEN
      v_name := r.walk_in_name; v_sub_type := 'Walk-in';
    ELSE
      v_name := 'Guest'; v_sub_type := 'Guest';
    END IF;
    v_class_count := v_class_count + 1;
    v_results := v_results || jsonb_build_object(
      'id', 'class-' || r.id, 'type', 'class', 'name', v_name,
      'time', r.checked_in_at,
      'subtitle', COALESCE(r.class_name, 'Class'), 'sub_type', v_sub_type
    );
  END LOOP;
  ...
  FOR r IN
    ...
    LEFT JOIN public.spa_services ss ON ss.id::text = sa.service_id
    ...
    WHERE sa.checked_in_at >= v_today_start AND sa.checked_in_at < v_today_end
  LOOP
    IF r.m_first IS NOT NULL THEN
      v_name := r.m_first || ' ' || r.m_last; v_sub_type := 'Member';
    ELSIF r.nmp_first IS NOT NULL THEN
      v_name := r.nmp_first || ' ' || r.nmp_last; v_sub_type := 'Non-Member';
    ELSE
      v_name := COALESCE(r.customer_first_name,'') || ' ' || COALESCE(r.customer_last_name,'');
      IF btrim(v_name) = '' THEN v_name := 'Spa Guest'; END IF;
      v_sub_type := 'Spa';
    END IF;
    v_spa_count := v_spa_count + 1;
    v_results := v_results || jsonb_build_object(
      'id', 'spa-' || r.id, 'type', 'spa', 'name', v_name,
      'time', r.checked_in_at,
      'subtitle', COALESCE(r.service_name, 'Spa'), 'sub_type', v_sub_type
    );
  END LOOP;

  SELECT count(*) INTO v_currently_in FROM public.check_ins
  ...
  RETURN jsonb_build_object(
    ...
    'stats', jsonb_build_object(
      ...
      'members', v_member_count, 'guests', v_guest_count,
      'classes', v_class_count, 'spa', v_spa_count
    )
  );
END;
$function$;