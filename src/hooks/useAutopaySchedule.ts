...
function formatCardInfo(
  ...
  const expStr =
    expMonth && expYear
      ? ` Exp ${String(expMonth).padStart(2, "0")}/${String(expYear % 100).padStart(2, "0")}`
      : "";
  return `${b} xxxx${l4}${expStr}`;
}
    ...
    queryFn: async (): Promise<{ entries: AutopayEntry[]; summary: AutopaySummary }> => {
      ...
      let paQuery = supabase
        ...
        .select(`
          ...
          member_id,
          members!inner(first_name, last_name, email, phone, membership_type, gender, 
      ...
      for (const pa of paymentAttempts || []) {
        ...
        entries.push({
          ...
          card_info: formatCardInfo(member?.card_brand, member?.card_last4, member?.card_exp_month, member?.card_exp_year),
      ...
      for (const m of activeMembers || []) {
        ...
        while (!isAfter(billingDate, rangeEnd)) {
          const inRange =
