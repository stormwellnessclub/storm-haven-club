...
BEGIN
  ...
  WHILE _current_date <= _end_date LOOP
    ...
    LOOP
      -- Check by schedule_id first (fast path for existing sessions from this schedule)
      ...
      IF _existing_count = 0 THEN
        -- Insert, but if a session already exists at (class_type, date, start_time)
        -- from a stale/inactive schedule, adopt it into the active one instead of failing.
...
-- Backfill: run reconciliation now so newly added schedules materialize
SELECT * FROM public.reconcile_and_generate_class_sessions(CURRENT_DATE, 6);