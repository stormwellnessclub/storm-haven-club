...
- Cron job changes are applied directly with the data tool (never a migration) because the job body embeds the project URL and internal token. Recovery record for Command 0A: job 2 `process-guest-feedback-emails` (`0 10 * * *`) was re-scheduled off the dropped `scheduled_functions_config` table onto the literal-URL + `x-internal-token` pattern. To restore any job, `cron.unschedule('<name>')` then `cron.schedule` with the same literal-URL + `x-internal-token` body used by all other jobs.
