 ...
 * - Suppressed for admins/staff (they may be impersonating)
...
export function SmsOptInGate() {
  ...
  // Respect a recent explicit opt-out (30 days)
  if (profile.sms_opt_out_at) {
    const optedOutAt = new Date(profile.sms_opt_out_at).getTime();
    const daysSince = (Date.now() - optedOutAt) / (1000 * 60 * 60 * 24);
  ...
  return (
    <Dialog open={true} onOpenChange={() => { /* non-dismissible */ }}>
      <DialogContent
        className="max-w-md"
        // Block the X-close button, overlay click, and Esc
