...
const cardVariants = cva(
  "rounded-lg border bg-card text-card-foreground transition-all duration-300",
  {
    variants: {
      variant: {
        default: "shadow-sm",
        elevated: "shadow-elevated border-border/50",
        glass: "bg-card/80 backdrop-blur-md border-border/50",
        luxury: "shadow-card hover:shadow-card-hover hover:-translate-y-1",
...
>(({ className, ...props }, ref) => (
  <h3
    ref={ref}
    className={cn(
      "font-serif text-xl sm:text-2xl font-semibold leading-none tracking-tight",
      className
    )}
    {...props}
  />
