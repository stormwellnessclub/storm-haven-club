...
BEGIN
  ...
  LOOP
    ...
    v_kind := CASE
      WHEN r.notes ILIKE 'First club visit%' THEN 'first_ever'
      WHEN r.notes ILIKE 'First visit as member%' THEN 'first_as_member'
      ELSE 'returning'
    END;
    v_results := v_results || jsonb_build_object(
      ...
      'sub_type', 'Member',
      'is_first_visit', v_kind <> 'returning',
      'first_visit_kind', v_kind
    );
  END LOOP;
  ...
  LOOP
    ...
    ELSIF r.nmp_first IS NOT NULL OR r.nmp_last IS NOT NULL THEN
      v_name := COALESCE(NULLIF(btrim(concat_ws(' ', r.nmp_first, r.nmp_last)), ''), r.nmp_email);
      v_sub_type := 'Non-Member • Spa Guest';
    ELSIF r.p_first IS NOT NULL OR r.p_last IS NOT NULL THEN
      v_name := COALESCE(NULLIF(btrim(concat_ws(' ', r.p_first, r.p_last)), ''), r.p_email);
      v_sub_type := 'Non-Member • Spa Guest';
    ELSE
