...
export const PT_BALANCE_DEFINITIONS = {
  ...
  unclosed: "Past sessions never marked done, with no package or payment. Close them out or apply a credit/payment.",
...
export function PTBalanceBreakdown({ userId, variant = "pt" }: { userId?: string; variant?: "pt" | "admin" }) {
  ...
  const items = [
    ...
    { label: "Past sessions not closed out", value: o?.unclosed_past_sessions_cents ?? 0, tip: PT_BALANCE_DEFINITIONS.unclosed, alert: true },
  ...
  return (
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5" data-testid="pt-balance-breakdown">
      {items.map((i) => (
