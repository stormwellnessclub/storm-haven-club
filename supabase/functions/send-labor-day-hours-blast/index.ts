// Labor Day (Mon Sep 7 2026) updated hours announcement email blast.
// Admin-only. Modes: preview (return HTML), testEmail (single send), else blast active members.
// Idempotent on email_type='labor_day_hours_2026_09_07'.
...
const TEMPLATE_KEY = "labor_day_hours_2026_09_07";
const TEST_TEMPLATE_KEY = "labor_day_hours_2026_09_07_test";
const SUBJECT = "Updated Labor Day hours — open 9:00 AM to 4:00 PM today";
  ...
  return `
    <div style="font-family:Georgia,'Times New Roman',Times,serif;max-width:600px;margin:0 auto;padding:0;">
      ...
      <div style="background:#ffffff;padding:32px 30px;border-left:1px solid #C1B19C;border-right:1px solid #C1B19C;color:#1C170F;">
        <p style="color:#a17e3a;font-family:Arial,sans-serif;letter-spacing:2px;font-size:11px;margin:0 0 8px;">AN UPDATE FROM THE CLUB</p>
        <h1 style="font-family:Georgia,serif;font-weight:500;margin:0 0 6px;font-size:26px;line-height:1.25;">Labor Day Hours &mdash; Updated</h1>
        <p style="color:#88766B;margin:0 0 22px;font-style:italic;">Monday, September 7th</p>

        <p>${greeting}</p>
        <p>Following yesterday&rsquo;s power outage, we&rsquo;re happy to share an update to our Labor Day schedule.</p>

        <div style="background:#F7F4EC;border:1px solid #E7DFCF;padding:20px;border-radius:6px;margin:14px 0 22px;text-align:center;">
          <p style="margin:0 0 6px;font-family:Arial,sans-serif;letter-spacing:2px;font-size:11px;color:#a17e3a;">MONDAY, SEPTEMBER 7TH</p>
          <p style="margin:0;font-size:24px;font-family:Georgia,serif;"><strong>Open 9:00 AM &ndash; 4:00 PM</strong></p>
          <p style="margin:8px 0 0;font-size:14px;color:#88766B;">One hour later than our originally announced Labor Day hours.</p>
        </div>

        <p>Classes, spa and recovery services, and the caf&eacute; will run within these hours. If you had a booking
        outside of this window, our team will reach out to you directly &mdash; you will not lose a credit or a session.</p>

        <p>Thank you for your patience through the outage. We&rsquo;ll keep you posted if anything changes.</p>
...
serve(async (req) => {
  ...
  if (body?.testEmail) {
    ...
    if (!(await isAllowedTestRecipient(req, email))) {
      return new Response(JSON.stringify({ error: "Test emails can only go to your own or a club address" }), {
