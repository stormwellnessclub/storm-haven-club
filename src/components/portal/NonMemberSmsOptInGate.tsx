...
/**
 * Non-dismissible interstitial that forces non-members (class-pass holders,
 * spa-only customers, etc.) to enable SMS notifications.
 *
 * Mirrors `SmsOptInGate` but reads/writes `non_member_profiles`.
 */
export function NonMemberSmsOptInGate() {
  const { user } = useAuth();
  ...
  if ((profile as any).sms_opt_in === true) return null;

  const optedOutAtRaw = (profile as any).sms_opt_out_at as string | null | undefined;
  if (optedOutAtRaw) {
    const daysSince =
      (Date.now() - new Date(optedOutAtRaw).getTime()) / (1000 * 60 * 60 * 24);
  ...
  const enable = async () => {
    ...
    const { error } = await (supabase.from("non_member_profiles") as any)
      .update({
        sms_opt_in: true,
        sms_opt_in_at: now,
      ...
      >
        <DialogHeader>
          ...
          <DialogDescription className="text-center pt-2">
            We use SMS for class reminders, waitlist promotions, spa appointment
            confirmations, and time-sensitive updates. This is how we make sure
            you never miss a class or appointment.
          </DialogDescription>
        </DialogHeader>
        ...
        <DialogFooter className="flex-col gap-2 sm:flex-col">
          {hasPhone ? (
            ...
            <Button asChild size="lg" className="w-full">
              <Link to="/portal/profile">Add phone number to continue</Link>
            </Button>
          )}
