...
export default function Support() {
  ...
  const handleCreateConversation = async () => {
    ...
    try {
      ...
      toast({ title: "Message sent", description: "Our team will respond to your message soon." });
    } catch {
  ...
  return (
    <MemberLayout>
      <div className="space-y-6">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Support & Concierge</h1>
          <p className="text-muted-foreground">
            Contact our team or request club services
          </p>
        </div>
                ...
                <DialogContent>
                  ...
                  <div className="space-y-4 py-4">
                    <div className="space-y-2">
                      ...
                      <Input id="subject" placeholder="What can we help you with?" value={newSubject} onChange={(e) => setNewSubject(e.target.value)} />
                    </div>
                    <div className="space-y-2">
                      <Label htmlFor="message">Message</Label>
                      <Textarea id="message" placeholder="Describe your question or concern..." rows={5} value={newInitialMessage} onChange={(e) => setNewInitialMessage(e.target.value)} />
                    </div>
                  </div>
                  <DialogFooter>
                    <Button onClick={handleCreateConversation} disabled={createConversation.isPending}>
                      {createConversation.isPending ? (<><Loader2 className="mr-2 h-4 w-4 animate-spin" />Sending...</>) : (<><Send className="mr-2 h-4 w-4" />Send Message</>)}
                    </Button>
                  </DialogFooter>
                </DialogContent>
              </Dialog>
            ...
            <div className="grid gap-6 md:grid-cols-3">
                  ...
                  <ScrollArea className="h-[500px]">
                    {loadingConversations ? (
                      ...
                      <div className="flex flex-col items-center justify-center py-8 px-4 text-center">
                        ...
                        <p className="text-sm text-muted-foreground">No conversations yet. Start one by clicking "New Message".</p>
                      </div>
                    ) : (
                      <div className="divide-y">
                        {conversations?.map((conversation) => {
                          ...
                          return (
                            <button key={conversation.id} onClick={() => setSelectedConversation(conversation.id)} className={`w-full text-left p-4 hover:bg-muted/50 transition-colors ${selectedConversation === conversation.id ? 'bg-muted' : ''}`}>
                              <div className="flex items-start justify-between gap-2">
                                <p className="font-medium text-sm line-clamp-1">{conversation.subject}</p>
                                <Badge variant={status.variant} className="shrink-0 text-xs">{status.icon}<span className="ml-1">{status.label}</span></Badge>
                              </div>
                              <p className="text-xs text-muted-foreground mt-1">{format(new Date(conversation.last_message_at), 'MMM d, h:mm a')}</p>
                            </button>
                          );
                        })}
                      </div>
              ...
              <Card className="md:col-span-2">
                <CardHeader className="pb-3">
                  ...
                  {selectedConv && <CardDescription>Started {format(new Date(selectedConv.created_at), 'MMMM d, yyyy')}</CardDescription>}
                </CardHeader>
                <CardContent className="p-0">
                  {!selectedConversation ? (
                    ...
                    <>
                      ...
                      <div className="p-4">
                        <div className="flex gap-2">
                          <Textarea placeholder="Type your message..." value={newMessage} onChange={(e) => setNewMessage(e.target.value)} rows={2} className="resize-none" onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleSendMessage(); } }} />
                          <Button onClick={handleSendMessage} disabled={!newMessage.trim() || sendMessage.isPending} className="shrink-0">
