...
CREATE POLICY "PT staff manage progress photos"
ON storage.objects FOR ALL TO authenticated
USING (bucket_id = 'pt-progress-photos' AND public.pt_is_staff_or_desk(auth.uid()))
WITH CHECK (bucket_id = 'pt-progress-photos' AND public.pt_is_staff_or_desk(auth.uid()));

CREATE POLICY "Clients view own progress photos"
ON storage.objects FOR SELECT TO authenticated
USING (bucket_id = 'pt-progress-photos' AND (storage.foldername(name))[1] = auth.uid()::text);
