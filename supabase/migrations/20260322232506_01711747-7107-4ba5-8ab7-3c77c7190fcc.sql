...
CREATE OR REPLACE FUNCTION public.get_admin_kids_care_bookings(p_booking_date date DEFAULT NULL::date, p_date_from date DEFAULT NULL::date, p_date_to date DEFAULT NULL::date, p_status text DEFAULT NULL::text, p_member_id uuid DEFAULT NULL::uuid, p_age_group text DEFAULT NULL::text)
 RETURNS TABLE(id uuid, member_id uuid, user_id uuid, child_name text, child_age integer, age_group text, booking_date date, start_time time without time zone, end_time time without time zone, status text, special_instructions text, checked_in_at timestamp with time zone, checked_out_at timestamp with time zone, checked_in_by uuid, checked_out_by uuid, parent_confirmed_pickup boolean, parent_confirmed_at timestamp with time zone, room text, created_at timestamp with time zone, updated_at timestamp with time zone, parent_first_name text, parent_last_name text, parent_email text, child_allergies text, child_medical_conditions text, child_medications text, child_emergency_contact_name text, child_emergency_contact_phone text, child_relationship_to_child text, child_authorized_pickup_persons text, child_special_instructions text, child_photo_release boolean, child_preferred_activities text)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  LEFT JOIN kids_care_children kc ON kc.user_id = b.user_id AND kc.full_name = b.child_name AND kc.is_active = true
  WHERE
