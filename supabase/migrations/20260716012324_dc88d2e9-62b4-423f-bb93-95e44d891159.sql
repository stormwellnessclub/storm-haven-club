...
CREATE INDEX IF NOT EXISTS class_schedules_effective_until_idx
  ON public.class_schedules (effective_until)
...
BEGIN
  ...
  -- Auto-deactivate schedules whose effective window has ended
  ...
  -- Hide sessions where schedule or class type is no longer active
  ...
  -- Unhide sessions if schedule/type became active again
  UPDATE class_sessions cs
  ...
  -- Sync empty sessions to their schedule's current values
...
REVOKE EXECUTE ON FUNCTION public.reconcile_and_generate_class_sessions(date, integer) FROM PUBLIC, anon, authenticated;
