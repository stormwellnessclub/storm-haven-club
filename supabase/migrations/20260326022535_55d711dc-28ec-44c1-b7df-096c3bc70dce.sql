...
-- Kiosk-safe check-in RPC: validates member status and creates check-in without requiring auth
...
BEGIN
  ...
  -- Find member by member_id, name, or email
  SELECT id, member_id, first_name, last_name, status, membership_type, photo_url
  INTO v_member
  FROM public.members
  ...
  -- Check if blocked
  IF EXISTS (SELECT 1 FROM public.blocked_persons WHERE member_id = v_member.id) THEN
    RETURN jsonb_build_object(
      'success', true,
      'access_granted', false,
  ...
  -- Check member status - only active members can check in
  IF v_member.status NOT IN ('active') THEN
    RETURN jsonb_build_object(
      'success', true,
      'access_granted', false,
      'denial_reason', 'inactive_membership',
      'member', jsonb_build_object(
        'first_name', v_member.first_name,
        'last_name', v_member.last_name,
        'membership_type', v_member.membership_type,
...
-- Kiosk-safe search RPC: returns only front-desk-safe fields
...
BEGIN
  ...
  -- Guest passes (today, active/purchased)
  FOR r IN
...
-- Kiosk attendance feed: today's check-ins with safe fields only
...
BEGIN
  ...
  SET checked_in_at = now(), status = 'completed'
  WHERE id = p_booking_id AND checked_in_at IS NULL;

  IF NOT FOUND THEN
    RETURN jsonb_build_object('success', false, 'error', 'Booking not found or already checked in');
  END IF;
...
BEGIN
  ...
  WHERE id = p_spa_id AND checked_in_at IS NULL;

  IF NOT FOUND THEN
    RETURN jsonb_build_object('success', false, 'error', 'Appointment not found or already checked in');
  END IF;
...
-- Grant anon access to all kiosk RPCs
GRANT EXECUTE ON FUNCTION public.kiosk_check_in_member(text) TO anon;
GRANT EXECUTE ON FUNCTION public.kiosk_search_visitors(text) TO anon;
GRANT EXECUTE ON FUNCTION public.kiosk_todays_attendance() TO anon;
GRANT EXECUTE ON FUNCTION public.kiosk_check_in_guest(uuid) TO anon;
GRANT EXECUTE ON FUNCTION public.kiosk_check_in_class(uuid) TO anon;
GRANT EXECUTE ON FUNCTION public.kiosk_check_in_spa(uuid) TO anon;
GRANT EXECUTE ON FUNCTION public.kiosk_check_in_member(text) TO authenticated;
...
GRANT EXECUTE ON FUNCTION public.kiosk_check_in_guest(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.kiosk_check_in_class(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.kiosk_check_in_spa(uuid) TO authenticated;
