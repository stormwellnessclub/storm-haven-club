...
RETURNS TABLE(
  ...
  booking_date date, start_time time without time zone, end_time time without time zone,
  status text, special_instructions text,
  checked_in_at timestamp with time zone, checked_out_at timestamp with time zone,
  checked_in_by uuid, checked_out_by uuid,
  parent_confirmed_pickup boolean, parent_confirmed_at timestamp with time zone,
  room text, created_at timestamp with time zone, updated_at timestamp with time zone,
  ...
  pass_id uuid, pass_type text, pass_status text, pass_classes_remaining integer,
  pass_classes_total integer, pass_purchased_at timestamp with time zone, pass_expires_at timestamp with time zone
)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SELECT
    ...
    cp.purchased_at::timestamp with time zone AS pass_purchased_at,
    cp.expires_at::timestamp with time zone AS pass_expires_at
