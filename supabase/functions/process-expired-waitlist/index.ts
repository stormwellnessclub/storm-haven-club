...
async function validateRequest(req: Request, _supabase: any): Promise<boolean> {
  // PHASE 0A: only the private internal task token, the service-role key, or a
  // staff JWT may run this job. Public anon/publishable keys are rejected.
  const auth = await requireTrustedCaller(req);
  return auth.ok;
}
